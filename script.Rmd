...
source("function/data_catch.wgt_sp.as.column.R")
...
Survey.data_catch_wgt <- data_catch.wgt_sp.as.column(HL_df) %>%
  mutate(Area = ifelse(lati > 48,"CS","BoB"))

# Plot hauls
 plot_1 <- ggplot(Survey.data_catch_nbr)+
  geom_point(aes(x=long,y=lati,col=log(TotalNo_Merluccius_merluccius)))+
  ...
  xlab("")+ylab("")+
  ggtitle("Hake in number")+
  scale_color_distiller(palette = "Spectral")

 plot_2 <- ggplot(Survey.data_catch_wgt)+
  geom_point(aes(x=long,y=lati,col=log(CatchWgt_Merluccius_merluccius)))+
  geom_sf(data = mapBase) +
  coord_sf(xlim = c(-12,0), ylim = c(43,52), expand = FALSE) +
  theme_classic()+
  theme(panel.spacing.x = unit(6, "mm"),
